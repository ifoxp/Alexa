...
class ActionExecutor:
    ...
    def open_website(self, url, argument=None):
        if argument:
            # Створюємо безпечний для URL рядок
    ...
    def run_script(self, path):
        try:
            # CREATE_NO_WINDOW приховує консоль (тільки для Windows)
    ...
    def set_system_volume(self, argument):
        try:
            ...
            if not level_str:
                print("❌ Не вдалося розпізнати рівень гучності.")
                return
            
            level = int(level_str)
            if not 0 <= level <= 100:
                print("❌ Рівень гучності має бути від 0 до 100.")
                return
            ...
            print(f"🔊 Встановлено гучність на {level}%")

        except Exception as e:
            print(f"❌ Помилка при зміні гучності: {e}")
...
class CommandManager:
    ...
    def find_command(self, text):
        ...
        if commands_with_args:
            for phrase, command in commands_with_args.items():
                # Перевіряємо, чи є фраза-тригер на початку тексту з високою схожістю
                PARTIAL_THRESHOLD = 90
                text_prefix = text_lower[:len(phrase)]
                score = fuzz.ratio(text_prefix, phrase)

                if score >= PARTIAL_THRESHOLD:
                    # Видаляємо частину, що збіглася, щоб отримати аргумент
                    argument = text_lower[len(phrase):].strip()
                    if argument:
                        print(f"[DEBUG] Знайдено команду з аргументом: '{command['name']}' (Оцінка префіксу: {score}%)")
    ...
    def execute_command(self, command, argument=None):
        ...
        if action:
            ...
            print(f"❌ Невідомий тип команди: '{command_type}'")